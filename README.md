...
**界面是一页文档，不是一串气泡。** 自己写的和模型回的都在同一条流里，从左上往下排，没有气泡也没有左右分边，只靠字重区分（自己写的略重）。输入框就嵌在文档末尾：点屏幕任意处光标落到那儿。**收起键盘就是落笔** —— 没有发送按钮，blur 本身就是提交动作，写完收笔，那段原地定格，回答接着在下一行写出来。键盘上方留了个「完成」，它只管收键盘：空文档时没内容可滑，下滑收键盘那条路走不通，不留个出口就会把人困在键盘里。

切换记事和新建都是先清空 draft 再 blur，所以那两处不会误提交。

正文下面留半屏空白。一页纸本来就不会在最后一行戛然而止，而且这块空白是可点的 —— 想接着写点哪儿都行。它还顺带让短笔记也能滑动：不然内容不够长时 `scrollDismissesKeyboard` 根本触发不了，下滑收键盘那条路等于不存在。

模型正在写的时候，末尾跟一根呼吸的光标。它是独立的 View 而不是拼在文本里的字符 —— 字符做不了淡入淡出。代价是它只能跟在整个 Markdown 块之后：块级渲染没有办法把一个会动的 View 塞进富文本流的末尾，所以回答短的时候它紧贴文字，长到换行之后会落在行尾右侧。另外 Shape 没有文字基线，得手动按底部往下压 3pt，才和同一行的文字坐在一条线上。

首个 token 到达前 `streamingText` 是空的，这时画面上只剩这根光标 —— 正好替掉了原先那个转圈，更像「对方正在写」。
