...
**正文界面上几乎没有词。** 写字那一页上只有两处字样：顶栏的 QW!（产品名，也是空笔记的标题），和整页还空着时输入位置上的 `Start writing`。其余文案都换成了图形：截断提示是一个省略号，上下文余量是一条会填满的细线，时间是 `09/25/2026` 这样的纯数字。

有四处留了英文，都是「不说清楚就只能干等或乱猜」的地方：启动页照实说现在在读权重还是在准备 GPU（这两步的体感完全不同，统称一句 Loading 会让最难熬的那几秒像卡死）；加载失败把 llama.cpp 报的原话写出来，因为「文件坏了」和「内存不够」该做的事不一样；列表页的新建按钮带着 New text 标签；长按菜单的三项写全 Read aloud / Copy / Share。

最后那一项是改回来的。原先只给图标不给字 —— 但系统的 context menu 天生是「一行图标 + 一行字」的版式，只给 `Image`，那行字不会消失，只会变空，于是三行菜单看着像没加载出来。无字这条原则的前提是界面自己会说话：一个输入框、一根进度条，形状就是说明；而一个固定版式的系统菜单不会因为你少给了字就换个长相。
...
空白的笔记一进来就自动落笔，新开一页本来就是为了写东西。有内容的笔记不抢焦点，改成停在底部 —— 那种页面打开多半是回来看的，键盘一上来就吃掉半屏正文，而正文才是回来的理由。中间试过「所有笔记都落笔」，键盘挡正文那一下实在碍事。生成中也不抢：那时候输入框不在视图里，位置让给正在写出来的回答。焦点要等一拍再提：这一下多半跟列表页的收起动画一起发生，而转场当中提焦点系统会直接忽略，键盘根本不会上来。

正文下面永远留半屏空白。一页纸本来就不会在最后一行戛然而止，而这块空白同时是收笔的主入口 —— 没在写就落笔，正在写就收笔。它还顺带让短笔记也能滑动：不然内容不够长时 `scrollDismissesKeyboard` 根本触发不了。

这块留白跟「进笔记停在底部」是有冲突的：留白在，「内容的底部」就是这片空地的底部，所以 `defaultScrollAnchor(.bottom)` 用不了 —— 它会让开屏正对着一片空白。停在最后一行字上只能瞄 `bottomAnchor` 那个具体位置。试过拿掉留白去换那个确定性的 API，但留白本身是这个界面的一部分，不该为了实现方便牺牲掉。

代价是那次 `scrollTo` 得跨过加载页淡出的转场：正文高度还没算完的时候发出去，SwiftUI 会把它安静地吞掉，不生效也不报错。所以滚好几次 —— 内容没变时反复滚到同一处没有副作用。

开始写的时候会把下方那半屏留白滑出来，让落笔的位置尽量靠上 —— 否则光标贴着键盘，能看见的正文只剩一两行。

新建记事那颗按钮浮在列表页下方，而不是藏在右上角的菜单里：那是这一页最主要的动作。

**等第一个字的时候有颗跳动的点。** 它顶替的只是「收笔之后什么都没发生」那几秒 —— 首字延迟在这个尺寸的模型上也有一两秒，没有任何反馈的话跟没提交一样。第一个字一出来点就撤了：文字自己就是最好的进度指示，旁边再挂个点只是抢注意力。试过让它一直跟到文末、像光标那样随排版走，做得到（做成行内图片逐帧换图），但为此每秒要重解析十几次 Markdown，而那东西在有字的时候本来就没人看。
...
**别为了省启动时间去调 `n_gpu_layers`，这一条是花了一天买来的。** 一度把它从 99 调到 20，之后 `llama_init_from_model` 开始返回 NULL：刚装完那几次必然失败，开到第三四次才好。

20 不是「少用点 GPU」，是让模型横跨 Metal 和 CPU 两个后端 —— 要建两套缓冲、两套计算图，是 llama.cpp 里走得少得多的一条路。省下的那点启动时间，换来一个装完打不开的 app。

排查过程更值得记。先后猜了五次，前四次全错：mmap 换 DIRECT_IO、上下文和批大小减半、设备过热、后端没重新 init。每一次都是看着症状编一个听起来通顺的因果，然后按那个因果改代码 —— 而每一次「改完好像好了」都只是那一轮碰巧没复现。

有用的只有两件事。一是让 app 把数字打出来：失败当时 `os_proc_available_memory` 报 2024 MB 空闲、`thermalState` 是 cool，一条错误信息同时杀掉了「内存」和「过热」两个猜测。二是用的人自己指出「从 99 改 20 之后毛病就多了」—— 那是连续使用攒出来的观察，比任何一次推断都准。

改回 99 之后好了很多，但刚装完第一次还是打不开 —— 而且**按重试没用，重启 app 就好**。这个区别把范围一下缩到了进程级的状态上，代码里正好有一处：

```swift
if !Self.backendReady {
    llama_backend_init()
    Self.backendReady = true   // ← 不管立没立起来
}
```

`llama_backend_init` 全进程只跑一次。第一次没把 Metal 后端立起来的话，同一个进程里再怎么重试都是在用那个坏的；换个进程才会重新 init —— 于是「重启就好了」。加载失败会先 `llama_backend_free` 把后端推倒，再整轮重来一次，也就是在原地做一遍换进程才会做的事。

这条听起来也很通顺，但它是第四个错的。下一次反馈是「第一次永久卡死在 95%，连失败文案都没提示出来」—— 那就不是 `llama_init_from_model` 返回了 NULL，那是它**根本没返回**。所有围绕「失败之后怎么重试」的设计，解释的都是一个不存在的失败。它是一个耗时几分钟的同步调用。

真正的原因在构建脚本里，不在 app 里：

```bash
GGML_METAL_EMBED_LIBRARY=ON    # ← 这里
```

ON 的意思是把 Metal **源码**嵌进二进制，首次 `llama_init_from_model` 时现场 `newLibraryWithSource` 编译几百个 kernel。真机上要几分钟，编完的结果进系统着色器缓存 —— 所以第二次、第三次就秒开，所以「重装之后又不行」。这个形状和先前每一个猜测都吻合，也正因为吻合，四次都能编出一个说得通的故事。

改成 OFF：Metal 库在构建时就编好，`default.metallib` 随 framework 一起打进包，运行时只是加载一个文件。装完第一次就是秒开。脚本里加了一条硬校验 —— OFF 了却没在 framework 里找到 `.metallib` 就直接让构建失败，避免它悄悄退回运行时编译。两个 workflow 的缓存键也带上了这个开关，否则 CI 会把旧的 xcframework 原样恢复回来，改了等于没改。

其余留下的：重试那一圈当保险，以及错误文案不再断言原因、只报那两个数字。之前那句「内存不够」印在 2 GB 空闲旁边，把人往错方向带了整整一天。
