...
**建上下文会失败，而排查它的过程值得记下来。** 权重读完之后 `llama_init_from_model` 还要分配 KV cache、建计算图、首次运行还要编译 Metal 内核 —— 这一步会返回 NULL，而且是刚装完那几次必然失败、开到第三四次突然就好了。

先后猜过三次，全错：mmap 换 DIRECT_IO（不是读法问题）、上下文和批大小减半（不是尺寸问题）、设备过热（当时确实烫，但那是巧合）。三次都是看着症状编一个听起来通顺的因果，然后按那个因果改代码。

真正有用的一步是让 app 自己把数字打出来：失败当时 `os_proc_available_memory` 报 2024 MB 空闲，`thermalState` 是 cool。两个猜测同时出局，一条错误信息顶得上一天的推断。剩下的解释指向首次运行的着色器编译 —— 「攒够几次就再也不用编」正好对上那个症状。

所以重试的形状是：先按原尺寸多试几次、中间真的等一会儿（编译需要时间，立刻重试等于没等），缩小尺寸退到最后两档兜底。错误文案也不再断言原因，只报那两个数字 —— 之前那句「内存不够」把人往错方向带了整整一天。
