...
**界面上没有词。** 屏幕上唯一的字样是 QW，而它是产品名，不属于任何语言 —— 顶栏是它，还没写字的笔记也是它。其余原本的文案都换成了图形：截断提示是一个省略号，上下文余量是一条会填满的细线，加载失败是一个惊叹号加一个重试箭头，长按菜单是三个裸图标，时间是 `09/25/2026` 这样的纯数字。
...
**界面是一页文档，不是一串气泡。** 自己写的和模型回的都在同一条流里，从左上往下排，没有气泡也没有左右分边，只靠字重区分（自己写的略重）。输入框就嵌在文档末尾：点屏幕任意处光标落到那儿。**收起键盘就是落笔** —— 没有发送按钮，blur 本身就是提交动作，写完收笔，那段原地定格，回答接着在下一行写出来。收笔有三条路：点输入框以外任意处、按右上角的勾、往下滑。

切换记事和新建都是先清空 draft 再 blur，所以那两处不会误提交。

正文下面留半屏空白。一页纸本来就不会在最后一行戛然而止，而且这块空白是可点的 —— 没在写就落笔，正在写就收笔。它还顺带让短笔记也能滑动：不然内容不够长时 `scrollDismissesKeyboard` 根本触发不了，下滑收键盘那条路等于不存在。
...
**工具栏两个图标要对齐。** `line.3.horizontal` 和 `checkmark` 的字形高度和光学重心并不一致，直接摆上去一高一低；统一字号再塞进等大的方框才横平竖直。
...
上传需要仓库 secrets：`ASC_KEY_ID`、`ASC_ISSUER_ID`、`ASC_API_KEY_P8`；下载页另需 `FLY_API_TOKEN`。
