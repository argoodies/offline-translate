...
排查过程更值得记。先后猜了四次，前三次全错：mmap 换 DIRECT_IO、上下文和批大小减半、设备过热。每一次都是看着症状编一个听起来通顺的因果，然后按那个因果改代码 —— 而每一次「改完好像好了」都只是那一轮碰巧没复现。
...
`llama_backend_init` 全进程只跑一次。第一次没把 Metal 后端立起来的话，同一个进程里再怎么重试都是在用那个坏的；换个进程才会重新 init —— 于是「重启就好了」。现在加载失败会先 `llama_backend_free` 把后端推倒，再整轮重来一次，也就是在原地做一遍换进程才会做的事。第二轮要重读那半个 G，只在本来就打不开的时候付这个钱。
