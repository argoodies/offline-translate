...
actor LlamaBridge {
    ...
    func load(
        ...
        // 这一步曾经在刚装完那几次必然失败。真凶是 `gpuLayers` 被调到 20 —— 那让
        // 模型横跨 Metal 和 CPU 两个后端，而混合后端建上下文是条脆得多的路。
        // 改回 99 之后就不犯了，详见 `AppSettings.gpuLayers`。
        //
        // 这圈重试留着当保险。第一次就成功的话它一分钱不花，而这一步一旦失败，
        // 整个 app 就打不开 —— 值得留一层。先按原尺寸带延迟多试几次（有些失败
        // 是一过性的，立刻重试等于没等），最后两档才缩小尺寸兜底。
