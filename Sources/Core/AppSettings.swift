...
enum AppSettings {
    ...
    ///
    /// **别再往下调。** 一度改成 20 想省启动时间，之后建上下文就开始失败 ——
    /// 刚装完那几次必然失败，开到第三四次才好。中间沿着「内存不够」「设备过热」
    /// 追了一整天，都不是。
    ///
    /// 20 不是「少用点 GPU」，是**让模型横跨两个后端**：20 层在 Metal，其余在 CPU。
    /// 混合后端要建两套缓冲、两套计算图，是 llama.cpp 里走得少得多的一条路。
    /// 建上下文栽在那条路上，比栽在内存上合理得多 —— 而且失败是从改这个数字
    /// 那一版开始出现的，时间线也对得上。
    ///
    /// 省下的那点启动时间，换来一个装完打不开的 app，这笔账怎么算都亏。
